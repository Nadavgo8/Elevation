...
let arr2 = users.filter((u) => u.address.zipcode.startsWith("5"));
//console.log(arr2);
...
//ex4
let arr4 = users.filter((u) => u.name.startsWith("C")).map((u) => u.name);
// console.log(arr4);
...
//ex6
const findBySuite = (suiteNo) => users.find((u) => u.address.suite === suiteNo);
// console.log(findBySuite("Apt. 950").company.name);

//ex7
const about = function (user) {
  users.forEach((u) => {
    console.log(
...
let total = inventory.reduce((sum, item) => {
...
let scoreCounts = studentScores.reduce((counts, score) => {
  if (score >= 90) {
    ++counts["A"];
  } else if (score >= 80) {
    ++counts["B"];
...
let totalPrice = cartItems.reduce((sum, item) => {
    ...
    if(item.category === "clothing"){
      ...
      priceAfterTax = item.price * 1.1;
        ...
        priceAfterTax = item.price * 1.05;
    }
    console.log(priceAfterTax);
  return sum + priceAfterTax*item.quantity;
}, 0);
